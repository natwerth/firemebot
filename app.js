...
(function () {
  ...
  async function run(q) {
    ...
    try {
      ...
      // Bubble up non-2xx with body snippet
      if (!resp.ok) {
        const text = await resp.text().catch(() => '');
        throw new Error(`Worker ${resp.status} ${resp.statusText}${text ? ` — ${text}` : ''}`);
      }

      // Be defensive: JSON when available, otherwise raw text
      ...
      if (ct.includes('application/json')) {
        ...
        data = { roast: text, raw: text, contentType: ct || 'text/plain' };
      }

      // Normalize common field aliases so rendering always works
      const normalized = {
        roast:
          (typeof data.roast !== 'undefined' ? data.roast :
          (typeof data.output !== 'undefined' ? data.output :
          (typeof data.result !== 'undefined' ? data.result :
          (typeof data.message !== 'undefined' ? data.message : null)))),
        title: data.title || data.input || query,
        riskTier: data.riskTier || data.risk || data.risk_level || null,
        rationale: data.rationale || data.reason || data.explanation || null,
      ...
      saveHistory(query);
      setStatus('Done.', 'ok');
    } catch (err) { console.error(err); setStatus(err && err.message ? err.message : 'Request failed', 'error'); const msg = String(err.message || err).toLowerCase();
