...
(function () {
  ...
  async function run(){
    ...
    (async () => {
      try {
        const rawObj = await callWorker(role); // may be object already
        const obj = (rawObj && typeof rawObj === 'object') ? rawObj : cleanToJson(String(rawObj || ''));
        if (!obj) {
          throw Object.assign(new Error('Could not parse JSON'), { details: String(rawObj).slice(0, 2000) });
        }
        ...
        // history + UI
        writeHistory(role, data);
        renderHistory(readHistory());

        hide(els.errorCard);
        setHtml(els.titleContent, escapeHtml(role));
        ...
        console.error(e);
        // Best-effort render last good
        const hist = readHistory();
        if (hist && hist.length) {
          const last = hist[0];
          if (last && last.payload) {
            renderGauge(last.payload.score);
            renderTiles(last.payload);
            wiresCopy(last.payload);
          } else {
